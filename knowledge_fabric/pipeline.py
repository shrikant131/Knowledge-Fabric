...
class KnowledgeFabricPipeline:
    def __init__(self, cfg):
        ...
        self.security=SecurityPolicy(cfg.allowed_sensitivity,cfg.allowed_users,cfg.allowed_groups,cfg.public_access); self.agent=KnowledgeFabricAgent(self); self.config_fingerprint=self._config_fingerprint()
    ...
    def query(self,question:str,principal:Principal|None=None)->dict:
        ...
        answer=self.generator.generate(system_prompt,user_prompt,max_tokens=self.cfg.max_tokens); usage=getattr(self.generator,"last_usage",{"input_tokens":0,"output_tokens":0})
        retrieved=[]; max_score=max((r.score for r in fused),default=1.0) or 1.0
        ...
        trace=[{"stage":"route","intent":intent.label},{"stage":"retrieve","hits":len(fused),"corrective_rounds":rounds},{"stage":"generation","provider":self.cfg.llm_provider if self.cfg.effective_llm_enabled() else "deterministic"},{"stage":"confidence","score":confidence.overall,"label":confidence.label}]
    ...
    def _retrieve_with_correction(self,question,qv,principal):
        ...
        while rounds<=self.cfg.max_correction_rounds:
            ...
            for q in queries[:self.cfg.max_agent_steps]:
                qvec=self.embedder.embed_one(q); lexical,vector=self._raw_retrieve(q,qvec,2+rounds,principal); lexical_all.extend(lexical); vector_all.extend(vector); allowed=self.security.allowed_chunk_ids(self.store.chunks,principal or Principal()); symbol_all.extend(symbol_match_results(q,[c for c in self.store.chunks if c.chunk_id in allowed],top_k=self.cfg.top_k*(2+rounds)))
